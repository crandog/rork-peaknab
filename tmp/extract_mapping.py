...
for c in contents:
    ...
    # Find assetName in the input string - it appears as "assetName":"value"
    # The inputArgsStr is a JSON string with nested JSON
    m = re.search(r'"assetName":\s*"([^"]+)"', inp)
...
for peak in sorted(mapping.keys()):
    print(f"  '{peak}': '{mapping[peak]}',")

print(f"\n=== FAILED ({len(failed)}) ===")
for p in failed:
    print(f"  {p}")

print(f"\nTotal: {len(mapping)} mapped, {len(failed)} failed")
